...
## Структура проекта
* `tomato.py` — бэкенд-сервер на FastAPI, отвечающий за загрузку картинок, предобработку матриц в NumPy (нормировка /255.0 и расширение размерности под батч) и инференс модели.
* `alert.html` — минималистичный и аккуратный фронтенд-интерфейс для загрузки фотографий пользователем.
